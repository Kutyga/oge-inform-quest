...
export default function DailyReviewScreen({ questions, idx, input, setInput, error, checked, correct, onCheck, onNext, onSkip, onExit }) {
  ...
  return (
    <div style={{ padding: "8px 20px 0" }}>
      ...
      <h2 className="quest-heading" style={{ fontSize: 20, margin: "12px 0 4px" }}>Восстановление знаний</h2>
      <p style={{ color: TOKENS.textMuted, fontSize: 13, margin: "0 0 16px" }}>
        Без таймера — просто освежаем то, что уже проходил.
      </p>
      ...
      <div key={idx} className="quest-pop" style={{ background: TOKENS.surface, border: `1px solid ${TOKENS.border}`, borderRadius: 16, padding: "22px 20px" }}>
        <span style={{ display: "inline-block", background: TOKENS.surfaceLight, color: TOKENS.mint, fontSize: 12, fontWeight: 600, padding: "4px 10px", borderRadius: 8, marginBottom: 14 }}>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
        ...
        <button
          ...
          style={{ flex: 1, padding: "12px 16px", borderRadius: 12, background: TOKENS.mint, color: "#0B2F1B", fontWeight: 700, fontSize: 15 }}
        >
          {checked ? (idx + 1 >= total ? "Завершить" : "Дальше") : "Ответить"}
        </button>
      </div>
    </div>
  );
